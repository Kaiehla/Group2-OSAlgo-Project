document.addEventListener('DOMContentLoaded', function() {
  ...
  function getTrackNoAndScan() {
    var trackno = document.querySelector('.track-sequence').value;
    var ctrack = document.querySelector('.current-track').value;
  
    document.querySelector(".track-number").innerHTML = trackno;
    document.querySelector(".current").innerHTML = ctrack;
...
function scan() {
  ...
  for (var i = 0; i < size; i++) {
    if (arr[i] < ctrack) left.push(arr[i]);
    if (arr[i] > ctrack) right.push(arr[i]);
  }
...
//     "Order of execution = " + seek_sequence;
document.querySelector(".order").innerHTML = seek_sequence;

//   document.querySelector(".time").innerHTML = "Total Seek Time = " + seek;
document.querySelector(".time").innerHTML = seek;
